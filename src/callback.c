...
{
	gchar *command = NULL;
	gchar *advanced_editor = "sensible-editor"; //TODO: hardcoded --> set a preference
	GError *error = NULL;
	...
	{
		gchar * msgError = g_strconcat( "ERROR: ", command );
		set_status( msgError );
		g_clear_error(&error);
		g_free(command);
		g_free(msgError);
...
void on_help_about(void)
{
	const gchar *copyright = "Copyright \xc2\xa9 2004-2010 Tarot Osuji"
							 "\nmodified by oluc \xc2\xa9 2012"
							 "\nmodified by baltasarq \xc2\xa9 2014";
