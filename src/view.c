...
void set_status(gchar * msg)
{
	// Post it, removing the last one
	gtk_statusbar_pop( pub->mw->statusBar, pub->mw->statusBarContext );
	gtk_statusbar_push( pub->mw->statusBar, pub->mw->statusBarContext, msg );
}

void set_status_counts(GtkTextBuffer *textBuffer)
{
	static gchar msgBuffer[128];
	...
	if ( textBuffer != NULL ) {
		...
		linPos = gtk_text_iter_get_line(&iter) + 1;
		colPos = gtk_text_iter_get_line_offset(&iter) + 1;
		chars = gtk_text_buffer_get_char_count( textBuffer );

		// Get word count
		gtk_text_buffer_get_start_iter(textBuffer, &textStart);
		gtk_text_buffer_get_end_iter(textBuffer, &textEnd);
		text = gtk_text_buffer_get_text(textBuffer, &textStart, &textEnd, FALSE);
		text_stats( text, &words, &lines );
	}

	sprintf( msgBuffer, "%3d, %3d | %9dls | %9dws | %9dcs",
						linPos, colPos, lines, words, chars );
	set_status( msgBuffer );
}
...
static void cb_modified_changed(GtkTextBuffer *buffer, GtkWidget *view)
{
	...
	// Update status
	set_status_counts( buffer );

	// Determines whether it was modified
	modified_flag = gtk_text_buffer_get_modified(buffer);

	// Modify window title
	set_main_window_title( modified_flag );
